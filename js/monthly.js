            ...
            if (allRecords.length > 0) {
                ...
                recordsList.innerHTML = allRecords.map((r, idx) => {
                    ...
                    if (r.step === 1) {
                        ...
                        // hypotheses_json is either an array (old) or {cards: [...], ai_details: {...}} (new)
                        const hypoJson = r.hypotheses_json || {};
                        const cards = Array.isArray(hypoJson) ? hypoJson : (hypoJson.cards || []);
                        const hList = cards.map((h, i) => `<div style="margin-bottom:8px; padding:8px; background:#f8fafc; border-radius:6px; font-size:0.9rem;"><strong>【仮説${i+1}】</strong><br>${h.why1 || ''} -> ${h.why2 || '...'} -> ${h.why3 || '...'}<br>└ <span style="color:var(--primary)">支援:</span> ${h.support || 'なし'}</div>`).join('');
