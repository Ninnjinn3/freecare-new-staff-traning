...
- **合否判定基準（重要）**: 
    ...
    - **「不足点(missing_points)」がある＝「不合格(×)」ではない**。更なる向上のためのアドバイスとして「不足点」を挙げつつ、判定は「○」にしても良い。
...
- 専門的視点: 専門職としての視点から1つアドバイスを含めること。
`;
...
const JSON_OUT = `出力形式(JSON形式厳守):
{
  ...
  "missing_points": ["さらに良くするためのアドバイス（重箱の隅をつつかず、プロの視点で有益なもの）"],
