...
2. **具体性のチェック項目**: 以下の要素が含まれているかで判断する。全て必須ではないが多いほど高評価。
   ...
   - 本人の反応（表情・言葉・行動の変化）
3. **改善例は送られた文章をベースに書き直す**: 
   - 新人が書いた元の文章をそのまま使い、足りない情報だけを追加して書き直すこと。
   - 元の文章と全く違う別の例文を作るのはNG。
   - 「元の文章 + 足りなかった情報」の形にすること。
`;
...
## 回答形式（必ずこのJSON形式で回答してください）:
{
  ...
  "improvement_example": "【重要】新人が書いた元の文章をそのまま使い、足りない部分（場所・時間帯・本人の反応など）だけを書き足した改善版を書く。元の文章と全く違う文を作らないこと。合格時は空文字。"
}`;
