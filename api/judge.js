...
1. **文字数で判断しない**: 短くても具体的な状況が書かれていればOK。長くても中身が曖昧ならNG。
2. **具体性のチェック項目**: 以下の要素が含まれているかを確認する。全て必須ではないが、多いほど高評価。
   - いつ（時間帯・タイミング: 朝食後、入浴中、夜間など）
   - どこで（場所: 食堂、居室、フロア、トイレなど）
   - 誰が（対象者の名前や「本人」の記載）
   - どんな状況（何をしていた時、何が起きたか）
   - 本人の反応（表情・言葉・行動の変化）
3. **改善指摘は入力内容に基づく**: 定型文ではなく、「この入力に何が足りないか」を具体的に指摘する。
`;
...
## 回答形式（必ずこのJSON形式で回答してください）:
{
  ...
  "missing_points": ["この入力に具体的に何が足りないか1", "不足点2"],
  "improvement_example": "入力内容をベースに、不足している情報（場所・時間帯・本人の反応など）を追加した改善版。元の文を活かしつつ足りない部分だけ補う。合格時は空文字。"
}`;
...
→ 改善例: 「昼食後、食堂でAさんがお腹をさすりながら『お腹が痛い』と訴えた。表情はやや苦しそうで、いつもは完食するが今日は半分残していた」
